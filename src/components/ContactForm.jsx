...
export default function ContactForm() {
  ...
  function validate() {
    const e = {}
    if (!form.name) e.name = 'Bu alan zorunlu'
    if (!form.email || !form.email.includes('@')) e.email = 'Geçerli bir e-posta giriniz'
    if (!form.message) e.message = 'Lütfen bir mesaj yazın'
    return e
  }
  ...
  return (
    <section id="contact" className="py-12 md:py-16">
      <div className="container">
        <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="text-2xl font-semibold mb-6">İletişim</motion.h2>
        <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-4" noValidate>
          ...
          <div className="md:col-span-2 flex items-center gap-3">
            <button type="submit" className="px-6 py-3 rounded-lg bg-accent text-white shadow hover:shadow-lg transform-gpu hover:-translate-y-0.5 active:scale-95 transition">Mesaj Gönder</button>
            {sent && <div className="text-green-600">Mesaj kuyruğa alındı (demo)</div>}
          </div>
        </form>
        {showToast && (
          <div role="status" aria-live="polite" className="fixed right-6 bottom-6 bg-slate-800 text-slate-100 shadow-lg border border-white/6 rounded-md px-4 py-3">
            <div className="font-semibold">Mesaj gönderildi</div>
            <div className="text-sm text-slate-300">Teşekkürler — En kısa sürede yanıt vereceğim.</div>
          </div>
        )}
      </div>
    </section>
